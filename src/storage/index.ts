...
export const initializeDB = async (): Promise<void> => {
  if (config.postgresEnabled) { // Use Postgres
    ...
    // await pgDb.runCreate('Drop INDEX if exists cycles_idx');
    await pgDb.runCreate('CREATE INDEX if not exists cycles_idx ON cycles (counter DESC)')
    await pgDb.runCreate(
      'CREATE TABLE if not exists accounts (accountId TEXT NOT NULL UNIQUE PRIMARY KEY, cycle BIGINT NOT NULL, timestamp BIGINT NOT NULL, ethAddress TEXT NOT NULL, account JSONB NOT NULL, accountType INTEGER NOT NULL, hash TEXT NOT NULL, isGlobal BOOLEAN NOT NULL, contractInfo JSONB, contractType INTEGER)'
    )
    ...
    // await pgDb.runCreate('Drop INDEX if exists accounts_idx');
    await pgDb.runCreate(
    ...
    // await pgDb.runCreate('Drop INDEX if exists transactions_hash_id');
    ...
    // await pgDb.runCreate('Drop INDEX if exists tokenTxs_idx');
    await pgDb.runCreate(
    ...
    // await pgDb.runCreate('Drop INDEX if exists tokens_idx');
    await pgDb.runCreate(
    ...
    // await pgDb.runCreate('Drop INDEX if exists receipts_idx');
    ...
    // await pgDb.runCreate('Drop INDEX if exists originalTxData_idx');
    await pgDb.runCreate(
    ...
    // await pgDb.runCreate('Drop INDEX if exists originalTxData2_idx');
    await pgDb.runCreate(
